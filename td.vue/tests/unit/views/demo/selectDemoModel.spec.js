...
describe('views/demo/SelectDemoModel.vue', () => {
    ...
    describe('selecting a demo model with desktop provider', () => {
        let demoModelItem;
        ...
        it('navigates to the desktop threat model page', () => {
            expect(mockRouter.push).toHaveBeenCalledWith(
                { name: 'desktopThreatModel', params: { threatmodel: 'Demo Threat Model' }}
            );
        });
    ...
    describe('selecting a demo model with gitlab provider', () => {
        let demoModelItem;

        beforeEach(async () => {
            mockStore = new Vuex.Store({
                state: {
                    provider: { selected: 'gitlab' }
                },
                actions: {
                    THREATMODEL_CLEAR: () => {},
                    THREATMODEL_LOAD_DEMOS: () => {},
